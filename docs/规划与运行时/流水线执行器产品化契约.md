...
## Execution graph progress records

The executor writes one atomic `state-store-v2` record before each real
project- or subject-level node invocation with `status=RUNNING`, then replaces
that record with its terminal result. A failed progress write prevents the
runner call. These records are observational evidence only and do not alter
scheduling, approval, ticket, gateway, algorithm, or artifact semantics.
