...
| `run_link` payload | SQLite record in `src/backend/app/services/mock_store.py` with fields including `run_link_id`, `project_id`, `reviewed_plan_id`, `run_id`, `dispatch_id`, `pipeline_path`, `summary_path`, `project_config_path`, `audit_id`, `status`, timestamps, warnings, and payload. `dispatch_id` binds the run to immutable Gateway evidence. |
