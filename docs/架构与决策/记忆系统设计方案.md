...
- 不固定某个供应商或模型名称。模型由现有 `MEDIMAGE_LLM_*` 配置选择，且 LLM 功能必须可关闭。
...
| 已有 background review 生成 proposed patch，并明确“proposal only”且禁止 PHI/raw imaging data | `src/backend/app/runtime/background_review.py:157-183` | 与候选记忆、人工确认模型一致 |
...
- background review 的 `proposed_memory_patch.md` 迁移为兼容 projection，不再直接成为真值。
