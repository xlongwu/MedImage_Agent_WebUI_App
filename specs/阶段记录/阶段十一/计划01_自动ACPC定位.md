...
- 当前能力仍是 `computed`：`estimated_ac_mm` 是本功能提供的前联合定位结果；`estimated_pc_mm` 仅用于建立 AC-PC 方向和右手坐标框架。两者都是模板反投影估计，不是人工解剖点的直接检测。

本轮静态审查**不能**证明科学验收完成。原完成条件中“独立参考集误差、失败率和 QC 误判”的证据尚未提供；在锁定人工标注集、评价协议、容差与失败判定之前，禁止将该能力标为 `validated` 或 Release Ready。

后续独立验证必须另建 Scientific Validation 任务，且至少覆盖：参考集与标注来源、前联合三维误差、作为坐标轴参考的后联合误差、失败率、95th percentile、QC 假阳性/假阴性、失败样本审查和能力矩阵更新。该任务不得改写原始 T1w 或 `rawdata/`。

## 一页决策

首版采用项目内 Python 的“模板刚体配准 + 参考点反投影”实现自动 AC-PC **估计和对齐**。功能目标是输出前联合估计坐标并建立以 AC 为原点的 ACPC 坐标框架；PC 估计只承担方向参考。该实现不调用 SPM、MATLAB、DPABI 或外部二进制，也不应宣称为人工解剖点的直接检测。

自动失败时返回结构化 QC 失败和人工复核需求，不回退到交互式桌面操作。
...
| 不做 | BOLD 直接定位前联合、深度学习训练、临床定位声明、自动修改源图像、桌面控制。 |

完成条件：

- [x] 合格 T1w 能生成重载成功的对齐 NIfTI、前联合估计坐标、AC-PC 方向参考和 transform。
...
3. 以变换的逆矩阵把参考 AC、PC 与中矢状面映射到 subject world 坐标；`estimated_ac_mm` 是前联合定位结果，`estimated_pc_mm` 仅作为坐标轴参考，字段保持 `estimated_*` 以避免误写为人工点。
...
| 步骤 | 修改 | 明确交付 |
|---|---|---|
| B1 | 修改 `planner/goal_contract_builder.py`、`planner/llm_planner.py` | 仅识别明确的“ACPC 对齐/前联合定位”目标；无 T1w 时产生补充信息决策，不生成空节点。 |
...
| B3 | 修改 `schemas/agent_task.py`、`services/agent_task_read_model.py`、对应前端 API/type/feature | 显示“前联合已估计/需复核/失败”，可下载 landmark JSON 和矩阵；不显示“人工定位完成”。 |
...
| `tests/unit/test_acpc_goal_planning.py` | “前联合定位”目标路由、T1 缺失时询问和 plan-only 零执行。 |
...
若修改 desktop surface，再运行 `npm --prefix desktop/electron run check`。每次 pytest 后按 `AGENTS.md` 安全清理根目录 `.pytest_cache/` 与 `.pytest_tmp*`，并复查 `git status --short`。
...
本计划只保留自动 AC-PC 前联合定位能力：`estimated_ac_mm` 是主要结果，`estimated_pc_mm` 是构建方向和坐标框架所需的辅助估计。当前结果是模板反投影的 `computed` 产物；若科学问题要求亚毫米级真实前联合点位，必须另立独立科学验证任务并使用目标人群和扫描协议的人工标注集验收。
