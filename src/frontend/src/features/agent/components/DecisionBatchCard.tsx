...
export function DecisionBatchCard({
  ...
  const validate = () => {
    ...
    batch.items.forEach((item) => {
      ...
      if (item.required && !value) {
        errors[item.item_id] = t("agent.decision.error.required");
        return;
      }
      if (!value) return;
  ...
  const setAnswer = (itemId: string, value: string) => {
    ...
    setLocalErrors((current) => {
      const { [itemId]: ignored, ...remaining } = current;
      return remaining;
    });
  };
      ...
      >
            ...
            <fieldset className={styles.decisionOptions} key={item.item_id}>
              ...
              <div className={styles.decisionMeta}>
                ...
                {item.recommendation_source ? (
                  <small>
                    {t("agent.decision.recommendationSource", {
        ...
        <div className={styles.actionFooter}>
          ...
          <Button data-primary-action="true" disabled={mutating} type="submit" variant="primary">
            {mutating ? t("agent.working") : t("agent.decision.batch.submit")}
          </Button>
        </div>
      </form>
    </Card>
