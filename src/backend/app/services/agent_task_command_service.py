"""Bounded create/answer/approve/cancel orchestration for Agent Tasks."""
...
_REALIGNMENT_PRODUCERS = frozenset(
    {
        "spm_realign_subject",
        "spm_smooth_subject",
...
def execution_prerequisite_issue(plan: dict[str, Any]) -> str | None:
    ...
    for node in nodes:
        ...
        stages = (
            params.get("stage_overrides")
            if isinstance(params.get("stage_overrides"), dict)
            else {}
        )
        # Native full preprocessing enables stages by default.  A reviewed
        # override may only disable a stage; therefore reject an explicit gap
        # in the real preprocessing chain before approving a ReHo run.
        if stages.get("reho") is not False:
            disabled_stages = [
                stage
                for stage in _NATIVE_REHO_PREREQUISITE_STAGES
                if stages.get(stage) is False
            ]
            if disabled_stages:
                return (
                    "ReHo execution requires the native preprocessing stages "
                    f"{', '.join(_NATIVE_REHO_PREREQUISITE_STAGES)}; explicitly "
                    f"disabled: {', '.join(disabled_stages)}."
                )

    reho_index = next(
        (index for index, node in enumerate(nodes) if str(node.get("id") or "") == "reho_subject"),
        None,
    )
    ...
    for node in nodes[: reho_index + 1]:
        ...
        if any(str(params.get(key) or "").strip() for key in _PREPROCESSED_INPUT_KEYS):
            return None
    return (
        "ReHo execution requires a realignment or smoothing producer, or an explicit "
        "reviewed preprocessed BOLD input."
    )


class AgentTaskCommandService:
    """Advance one command through a finite, non-autonomous transition set."""
    ...
    def __init__(
        ...
        *,
        ...
        plan_saver: Callable[..., Any] | None = None,
        dry_runner: Callable[..., dict[str, Any]] | None = None,
        executor: ReviewedExecutionService | None = None,
        summary_service: ApprovalSummaryService | None = None,
        conversion_checker: Callable[..., dict[str, Any]] | None = None,
        reconcile_once: Callable[..., Any] | None = None,
        ...
        self.summary_service = summary_service or ApprovalSummaryService()
        self.conversion_checker = conversion_checker or ReviewedConversionService().check_readiness
        ...
        if self.memory_context_service is None:
            ...
            if config.enabled and config.use_enabled:
                try:
                    self.memory_context_service = MemoryRetrievalService(
    ...
    def approve(
        ...
        *, project_id: str,
        ...
        dry_run = self.dry_runner(
            plan=reviewed.payload["plan"],
            approval=approval,
            project_id=project_id,
            reviewed_plan_id=reviewed.reviewed_plan_id,
            project_config_path=reviewed.project_config_path,
            actor=actor,
            lifecycle_id=lifecycle_id,
        )
        if not dry_run.get("ok"):
            blocked_status = str(dry_run.get("status") or "UNKNOWN")
            details: dict[str, Any] = {"blocked_status": blocked_status}
            blocking_issues = dry_run.get("errors") or dry_run.get("blocking_issues")
            if isinstance(blocking_issues, list):
                details["blocking_issues"] = [str(item) for item in blocking_issues]
            raise SafetyError(
                f"AGENT_DRY_RUN_BLOCKED: {blocked_status}",
                code="AGENT_DRY_RUN_BLOCKED",
                details=details,
            )

        result = self.executor.execute(
            ExecuteReviewedRequest(
                plan=reviewed.payload["plan"],
                approval=approval,
                project_id=project_id,
                reviewed_plan_id=reviewed.reviewed_plan_id,
        ...
        if not result.get("ok"):
            ...
            raise SafetyError(
                f"AGENT_EXECUTION_BLOCKED: {blocked_status}",
        ...
        *,
        ...
        candidate = next(
            (
                ...
                if item.candidate_id == proposal.recommended_candidate_id and item.eligible
            ),
            None,
        )
        if candidate is None:
            raise SafetyError("RECOVERY_CANDIDATE_NOT_ELIGIBLE", code="RECOVERY_CANDIDATE_NOT_ELIGIBLE")
        recovery = RecoveryExecutionService(self.store)
        recovery.approve(
            project_id=project_id,
            lifecycle_id=lifecycle_id,
            proposal_id=proposal.recovery_proposal_id,
            candidate_id=candidate.candidate_id,
            command_id=f"{command_id}:approve",
            actor=actor,
        )
    ...
    def _apply_science_answers(
        ...
        signals = AgentTaskCommandService._science_signals(copied, project_metadata)
    ...
    def _conversion_readiness(self, plan: dict[str, Any]) -> dict[str, Any]:
        for node in plan.get("nodes", []):
            if not isinstance(node, dict) or node.get("id") != ReviewedConversionService.NODE_ID:
                continue
            params = node.get("params") if isinstance(node.get("params"), dict) else {}
    ...
    def _dry_run(
        ...
        *,
        ...
        if approval is None:
            nodes = tuple(str(node.get("id")) for node in plan.get("nodes", []) if isinstance(node, dict) and node.get("id"))
            backends = tuple(str(node.get("backend")) for node in plan.get("nodes", []) if isinstance(node, dict) and node.get("backend"))
            approval = self.summary_service._confirmations(plan=plan, node_ids=nodes, backend_ids=backends)
