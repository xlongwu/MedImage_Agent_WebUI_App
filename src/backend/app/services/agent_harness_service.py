...
class AgentHarnessService:
    ...
    def run_one(self, *, lifecycle, actor: str, lease_owner: str | None = None) -> HarnessRunResult:
        ...
        claimed = self.store.update_agent_harness_attempt(claimed, expected_status="RUNNING")
        ...
        if prior is not None:
            return HarnessRunResult(lifecycle=lifecycle, attempt=self._stop_claimed(claimed, "AGENT_HARNESS_DUPLICATE_STEP"))
    ...
    def _propose_with_one_repair(
        ...
        try:
            ...
            if attempt.model_calls_used + 2 > self.config.max_model_calls:
                raise RuntimeError("AGENT_MODEL_OUTPUT_INVALID")
            try:
                return (
                    self.adapter.propose_action(
    ...
    def _apply(self, envelope: ActionEnvelope, lifecycle, actor: str) -> tuple[object, str, str | None]:
        if envelope.kind == "draft_plan":
            ...
            status = "WAITING_FOR_USER" if result.state in {"WAITING_FOR_INPUT", "WAITING_FOR_SCIENCE_DECISION"} else "FINISHED"
            return result, status, "WAITING_FOR_APPROVAL" if result.state == "WAITING_FOR_APPROVAL" else None
        if envelope.kind == "request_decision":
        ...
        if envelope.kind == "propose_recovery":
            ...
            return result, "FINISHED", "RECOVERY_PROPOSED"
    ...
    def _claim(self, attempt: AgentHarnessAttempt, owner: str) -> AgentHarnessAttempt | None:
        ...
        try:
            return self.store.update_agent_harness_attempt(claimed, expected_status=expected)
        except RuntimeError:
            return None
    ...
    def _complete_claim(
        ...
        *,
        ...
        return self.store.update_agent_harness_attempt(updated, expected_status="RUNNING")
